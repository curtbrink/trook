<template>
  <v-container class="mx-auto d-flex align-center justify-center">
    <v-row>
      ...
      <v-col cols="8">
        <v-card>
          <v-card-title>File Ingestion</v-card-title>
          <v-card-text>
            <v-container fluid>
              <v-row>
                <v-col cols="4">
...
import { useFileIngestionStore } from "@/stores/file-ingestion.store.ts";

const snackbar = useSnackbarStore();
const driverJobsStore = useDriverJobsStore();
const playerJobsStore = usePlayerJobsStore();
const fileStore = useFileIngestionStore();
...
const filePicked = async () => {
  ...
  try {
    await fileStore.postJobs(formData);
